...
trait GenericLiftedMongoInvoker[T]  {
  ...
  protected def attributes: Seq[(String)] = queryNode match {  // todo for now result without a type
    case TableExpansion(_,_,pn: ProductNode) => pn.nodeChildren.map(_.asInstanceOf[Select].field.name) //zip pn.buildType.children  // todo build type is now protected resolve problem
